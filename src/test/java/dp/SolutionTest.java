...
public class SolutionTest {
    ...
    public void testStoneGame() {
        ...
        /**
         * 此处不能选择4，要考虑到 选择当前最大值的同时还要考虑对手的值
         * 当   end - start+1 > 4
         *      if(a[i+1]>a[i]&&a[i]+a[end] > a[i+1]+a[end-1]){
         *
         *      }else{
         *
         *      }
         *     a[i]+a[end] > a[i+1]+a[end-1]
         */
        System.out.println();
        //10 4
        //16 5
        int[] case2 = {3, 2, 10, 4};
        assertEquals(true, solution.stoneGame(case2));

        int[] caseReturn = {4, 10, 2, 3};
        assertEquals(true, solution.stoneGame(caseReturn));

        System.out.println();
        int[] myCase = {162,259,203,207,455,483,262,49,280,379};
        assertEquals(true, solution.stoneGame(myCase));
...
//        assertEquals(true, solution.stoneGame(case3));
    }
}
